...
fn parser_requires_decimal_nonzero_uid_and_initial_assertion() {
    for args in [
        ...
        vec!["admin", "activation-anchor", "provision", "--uid", "1000"],
    ] {
...
#[test]
fn parser_accepts_only_the_fixed_locator_inputs() {
    let help = degu()
...
fn non_root_execution_is_refused_without_using_generic_root_override() {
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("requires effective UID 0"));
}
