...
Sealed staging admits one finding at a time within fixed inventory bounds: 1,023 total directories (root inclusive), 100,000 entries, depth 128, 16 MiB of path bytes, 64 MiB of manifest accounting, and a 1 GiB content budget per verification pass. The clean preview runs the same admission checks and discloses any limit rejection before execution. Once proof schema v3 records exist in the seal WAL, older degu binaries cannot read them; operational downgrade is not supported.
