...
The account home itself is an account-database fact, never `HOME`. A statically linked build reads only the local account file, because the name service switch resolves LDAP, SSSD, and winbind through plugins that a static image cannot load, and a build that cannot see those services reports such an account as absent rather than as unreachable. When the local file has no entry for the effective UID, degu therefore puts the same question to the host's own resolver: it runs `getent passwd` from a fixed absolute path with a fixed argument list, an emptied environment, a neutral working directory, a closed standard input, and bounds on both time and output, and accepts the returned record only when its UID column is the UID that was asked for. That consults the same account database a dynamically linked build would consult in-process, so it can name no path the account database does not already name, and no environment variable can redirect it. A host with no such resolver has no name service switch either, so there the local file's answer was already complete and a miss is a genuinely missing account.
...
Purge removes degu-managed directory entries. Regular-file hard links stage only when the complete link group lives inside the selected tree; degu re-observes every alias and refuses external or unenumerable topology before any mutation. Ordinary regular-file extended attributes (Linux `user.*`; common macOS attributes such as quarantine, provenance, Finder metadata, and resource forks) are bound into the staged content proof and re-verified on undo and purge, while authority-bearing namespaces, ACLs, capabilities, symlink attributes, and special files remain fail closed. Sealed permanent purge does not support multi-link groups or proof-bound extended attributes: a direct `clean --purge` stages such a tree and reports that permanent deletion was not admitted, and `trash purge` and expiry retain those entries — still undoable — while continuing to delete unrelated supported entries. Bytes reported as hardlink-shared may remain allocated through links outside a purged tree, so the amount deleted is not a guarantee of equal quota release; human output calls out this case, and JSON exposes it as `bytes_hardlinked`.

Sealed staging admits one finding at a time within fixed inventory bounds: 1,023 total directories (root inclusive), 100,000 entries, depth 128, 16 MiB of path bytes, and 64 MiB of manifest accounting. Regular-file and symlink payload bytes have no fixed aggregate ceiling; they are streamed through held descriptors while content proof is computed. Ordinary regular-file xattr values retain a separate 1 GiB aggregate proof budget because each value must be materialized for the proof hash. The clean preview runs the same admission checks and discloses any limit rejection before execution. Once proof schema v3 records or WAL schema v12 frames exist in the seal WAL, older degu binaries cannot read them; operational downgrade is not supported.
