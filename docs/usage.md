...
The preview does not modify data. Running `degu clean` without `--purge` stages the current **Ready to clean** findings; staged entries later expire under the [seven-day purge policy](safety.md#staging-undo-and-purge). Sources outside HOME are supported when degu can create or validate a private trash domain on the same certified local mount; cross-mount copy/delete remains forbidden. The authority anchor and WAL store may be on different mounts from that data domain.
