...
The target and each ancestor are resolved without following symlinks, and the target's parent must already exist and be a trusted namespace — owned by you and not group- or other-writable. `--init` does not create missing ancestor directories, and it refuses a parent another user could substitute; create such a parent yourself first (for example `mkdir -m 700 -p /scratch/$USER`).

`--init` guarantees ownership and modes only at initialization time. Cache tools you run later create their own descendants under your ambient umask, so with `umask 002` or `007` those descendants are group-writable (`0775`/`0770`). degu's scan and clean stay conservative about group-writable trees, so such a populated cache is reported but not eligible for cleaning until a scoped cooperative-group trust policy lands. `--init` does not by itself make a group-writable relocated cache cleanable.

Initialization is transactional over the filesystem: degu validates the full relocation plan before creating anything, and on any initialization failure it prints no sourceable exports and removes only entries created by that invocation whose recorded identities still match — a rollback failure is reported with every known residual path. The transaction ends once every root is initialized; if writing the report or script then fails, the completed roots are left in place, since they already form a valid, idempotent state that a re-run reports as already initialized. With `--init --json`, the otherwise unchanged relocation report also includes an `initialization` object listing newly created and already initialized roots.
