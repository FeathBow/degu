...
The preview does not modify data, and it is honest about execution: every planned finding passes through the same metadata-only sealed-staging admission assessment that production re-verifies. Findings that sealed staging cannot execute are moved out of the **Ready to clean** totals into explicit **Blocked by sealed staging preflight** or **Needs execution validation** groups with their reasons, and `--json` reports a `staging_preflight` entry aligned with each planned item. An explicit `--path`/`--review` selection is assessed atomically: if any selected item fails preflight, the whole batch is rejected before any source, trash, or WAL mutation and the remaining items report `not_attempted`.

Running `degu clean` without `--purge` stages the current **Ready to clean** findings; staged entries later expire under the [seven-day purge policy](safety.md#staging-undo-and-purge). Sources outside HOME are supported when degu can create or validate a private trash domain on the same certified local mount; cross-mount copy/delete remains forbidden. The authority anchor and WAL store may be on different mounts from that data domain.
